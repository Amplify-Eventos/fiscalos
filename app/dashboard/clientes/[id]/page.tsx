...
export default async function ClienteDetalhesPage({
  ...
  const economiaMaxima =
    simulacoes.length > 0 && simulacoes[0].economiaVsAtual > 0
  ...
  const porcentagemSublimite =
    revenue12m > 0 ? Math.min((revenue12m / sublimiteSimples) * 100, 100) : 0;
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600">
              CNPJ: {client.cnpj} | Faturamento: R${" "}
              {Number(client.revenueLast12m).toLocaleString("pt-BR")}
            </p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-500">
                    {currentTotalTax > 0
                      ? `R$ ${currentTotalTax.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`
                      : "Não info"}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">
                    Carga:{" "}
                    {currentTotalTax > 0 && revenue12m > 0
                      ? ((currentTotalTax / revenue12m) * 100).toFixed(1)
                      : 0}
                    %
                  </p>
                </div>
              ...
              <div className="mt-3 text-xs space-y-1 pt-3 border-t">
                {Number(client.currentDAS || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">DAS</span>
                    <span className="font-medium">
                      R${" "}
                      {Number(client.currentDAS).toLocaleString("pt-BR", {
                        maximumFractionDigits: 0,
                      })}
                    </span>
                  </div>
                )}
                {Number(client.currentIRPJ || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">IRPJ/CSLL</span>
                    <span className="font-medium">
                      R${" "}
                      {(
                        Number(client.currentIRPJ) + Number(client.currentCSLL)
                      ).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                    </span>
                  </div>
                )}
                {Number(client.currentPIS || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">PIS/COFINS</span>
                    <span className="font-medium">
                      R${" "}
                      {(
                        Number(client.currentPIS) + Number(client.currentCOFINS)
                      ).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                    </span>
                  </div>
                )}
                {Number(client.currentISS || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">ISS</span>
                    <span className="font-medium">
                      R${" "}
                      {Number(client.currentISS).toLocaleString("pt-BR", {
                        maximumFractionDigits: 0,
                      })}
                    </span>
                  </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex justify-between items-start">
                ...
                <AlertCircle
                  className={`h-5 w-5 ${fatorR >= 28 ? "text-green-500" : "text-red-500"}`}
                />
              </div>
              <div className="mt-3 pt-3 border-t">
                <div className="w-full bg-slate-100 rounded-full h-1.5 mb-1">
                  <div
                    className={`h-1.5 rounded-full ${fatorR >= 28 ? "bg-green-500" : "bg-red-500"}`}
                    style={{ width: `${Math.min(fatorR, 100)}%` }}
                  ></div>
                </div>
          ...
          <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50 to-white">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-green-600">
                R${" "}
                {economiaMaxima.toLocaleString("pt-BR", {
                  maximumFractionDigits: 0,
                })}
              </p>
            ...
            <CardContent className="pt-4 pb-4 flex items-center justify-between">
              <div className="w-1/3">
                <p className="text-sm font-semibold text-amber-900">
                  Alerta de Sublimite (R$ 3,6M)
                </p>
                <p className="text-xs text-amber-700">
                  Faturamento acumulado: R$ {revenue12m.toLocaleString("pt-BR")}
                </p>
              </div>
              <div className="w-2/3 pl-4">
                <div className="w-full bg-amber-200 rounded-full h-2.5 mb-1">
                  <div
                    className={`h-2.5 rounded-full ${porcentagemSublimite > 90 ? "bg-red-500" : "bg-amber-500"}`}
                    style={{ width: `${porcentagemSublimite}%` }}
                  ></div>
                </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-slate-600 uppercase bg-slate-100">
                    <tr>
                      ...
                      <th className="px-6 py-4 bg-red-50/80 text-red-900 border-x border-red-100">
                        Cenário Atual (
                        {client.taxRegime?.replace("_", " ") || "SIMPLES"})
                      </th>
                      <th className="px-6 py-4 bg-green-50/80 text-green-900">
                        Cenário Ideal ({simulacoes[0].regime.replace("_", " ")})
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    <tr className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 border-x border-slate-100">
                        R${" "}
                        {revenue12m.toLocaleString("pt-BR", {
                          maximumFractionDigits: 0,
                        })}
                      </td>
                    ...
                    <tr className="hover:bg-slate-50 text-slate-500">
                      <td className="px-6 py-3 pl-10 flex items-center">
                        <div className="w-2 h-2 rounded-full bg-slate-300 mr-2"></div>{" "}
                        DAS / Simples
                      </td>
                      <td className="px-6 py-3 border-x border-slate-100">
                        R${" "}
                        {Number(client.currentDAS || 0).toLocaleString(
                          "pt-BR",
                          { maximumFractionDigits: 0 },
                        )}
                      </td>
                      <td className="px-6 py-3">
                        R${" "}
                        {simulacoes[0].regime === "SIMPLES_NACIONAL"
                          ? simulacoes[0].impostoTotal.toLocaleString("pt-BR", {
                              maximumFractionDigits: 0,
                            })
                          : "0"}
                      </td>
                    ...
                    <tr className="hover:bg-slate-50 text-slate-500">
                      ...
                      <td className="px-6 py-3">
                        R${" "}
                        {simulacoes[0].regime !== "SIMPLES_NACIONAL"
                          ? (simulacoes[0].impostoTotal * 0.4).toLocaleString(
                              "pt-BR",
                              { maximumFractionDigits: 0 },
                            )
                          : "0"}{" "}
                    ...
                    <tr className="hover:bg-slate-50 text-slate-500">
                      ...
                      <td className="px-6 py-3">
                        R${" "}
                        {simulacoes[0].regime !== "SIMPLES_NACIONAL"
                          ? (simulacoes[0].impostoTotal * 0.3).toLocaleString(
                              "pt-BR",
                              { maximumFractionDigits: 0 },
                            )
                          : "0"}{" "}
                    ...
                    <tr className="bg-slate-50 font-semibold">
                      ...
                      <td className="px-6 py-4 border-x border-slate-200 text-red-700">
                        {revenue12m > 0
                          ? ((currentTotalTax / revenue12m) * 100).toFixed(2)
                          : 0}
                        %
                      </td>
                      <td className="px-6 py-4 text-green-700">
                        {revenue12m > 0
                          ? (
                              (simulacoes[0].impostoTotal / revenue12m) *
                              100
                            ).toFixed(2)
                          : 0}
                        %
                    ...
                    <tr className="bg-red-50/30">
                      ...
                      <td className="px-6 py-4 text-red-600 font-bold border-x border-red-100 text-lg">
                        R${" "}
                        {currentTotalTax.toLocaleString("pt-BR", {
                          maximumFractionDigits: 0,
                        })}
                      </td>
                    ...
                    <tr className="bg-green-100/50 border-t-2 border-green-200">
                      ...
                      <td className="px-6 py-5 font-black text-green-700 text-2xl">
                        + R${" "}
                        {simulacoes[0].economiaVsAtual.toLocaleString("pt-BR", {
                          maximumFractionDigits: 0,
                        })}
                      </td>
                    </tr>
                ...
                <div className="w-full md:w-1/3">
                  <div className="relative pt-4 text-center">
                    ...
                    <div className="flex justify-between text-xs font-bold text-slate-400">
                      <span>0%</span>
                      <span className="text-indigo-600 relative -left-4">
                        Meta 28%
                      </span>
                      <span>100%</span>
                    </div>
                  ...
                  {fatorR >= 28 ? (
                    <div className="bg-green-50 rounded-lg p-6 border border-green-100">
                      ...
                      <p className="text-sm text-green-700">
                        A folha de pagamento atual (R${" "}
                        {payroll12m.toLocaleString("pt-BR", {
                          maximumFractionDigits: 0,
                        })}
                        ) já representa 28% ou mais do faturamento. A tributação
                        dos serviços está na alíquota inicial de 6% (Anexo III)
                        e não em 15.5% (Anexo V).
                      </p>
                    </div>
                  ) : (
                    <div className="bg-amber-50 rounded-lg p-6 border border-amber-200">
                      ...
                      <p className="text-sm text-amber-800 mb-4">
                        ...
                        ) está abaixo de 28% do faturamento (R${" "}
                        {revenue12m.toLocaleString("pt-BR", {
                          maximumFractionDigits: 0,
                        })}
                        ).
                      ...
                      <div className="bg-white p-4 rounded border border-amber-100 mt-2">
                        ...
                        <p className="text-slate-600 text-sm">
                          Para atingir 28% e cair do Anexo V (15.5%) para o
                          Anexo III (6%), o Pró-Labore/Folha acumulado de 12
                          meses deve ser de no mínimo{" "}
                          <strong>
                            R${" "}
                            {(revenue12m * 0.28).toLocaleString("pt-BR", {
                              maximumFractionDigits: 0,
                            })}
                          </strong>
                          .
                        ...
                        <p className="text-indigo-700 text-sm font-medium mt-2">
                          Faltam R${" "}
                          {(revenue12m * 0.28 - payroll12m).toLocaleString(
                            "pt-BR",
                            { maximumFractionDigits: 0 },
                          )}{" "}
                          na folha anual (aprox. R${" "}
                          {(
                            (revenue12m * 0.28 - payroll12m) /
                            12
                          ).toLocaleString("pt-BR", {
                            maximumFractionDigits: 0,
                          })}
                          /mês) para garantir a alíquota menor.
                        </p>
                      </div>
                    </div>
                  )}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {client.taxRegime?.replace("_", " ") || "Simples Nacional"}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Porte</span>
                <span className="font-medium">
                  {client.companySize || "ME"}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                ...
                <CardContent>
                  {diagnostico.problemasDetectados.length > 0 && (
                    <div className="mb-4">
                      <p className="text-sm font-medium text-red-600 mb-2 flex items-center">
                        <AlertTriangle className="h-4 w-4 mr-1" /> Pontos de
                        Atenção:
                      </p>
                      <ul className="space-y-2">
                        {diagnostico.problemasDetectados.map(
                          (problema: string, idx: number) => (
                            <li
                              key={idx}
                              className="flex items-start text-sm text-slate-700 bg-red-50 p-2 rounded"
                            >
                              {problema}
                            </li>
                          ),
                  ...
                  {diagnostico.oportunidadesIdentificadas.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-2">
                        {diagnostico.oportunidadesIdentificadas.map(
                          (oportunidade: string, idx: number) => (
                            <li
                              key={idx}
                              className="flex items-start text-sm text-slate-700 bg-green-50 p-2 rounded border border-green-100"
                            >
                              {oportunidade}
                            </li>
                          ),
