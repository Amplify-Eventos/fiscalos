...
function mapClientToTwin(client: any) {
  ...
  return {
    ...
    custos: {
      ...
      total:
        Number(client.payrollLast12m || 0) +
      ...
      for (const client of clients) {
        ...
        if (rev > 0) {
          ...
          for (const prob of diagnostico.problemasDetectados) {
            if (prob.includes("Fator R") && client.alertas.length < 3)
              client.alertas.push("Fator R Baixo");
          }

          for (const est of estrategias) {
            if (
              est.id === "desenquadramento-mei" &&
              !client.alertas.includes("Estourou MEI")
            )
              client.alertas.push("Estourou MEI");
            if (
              est.id === "transicao-presumido" &&
              !client.alertas.includes("Risco Sublimite")
            )
              client.alertas.push("Risco Sublimite");
            if (
              est.id === "revisao-ncm" &&
              !client.alertas.includes("Recup. NCM")
            )
              client.alertas.push("Recup. NCM");
            if (
              est.id === "equiparacao-hospitalar" &&
              !client.alertas.includes("Equiparação Hosp.")
            )
              client.alertas.push("Equiparação Hosp.");
            if (
              est.id === "mudar-regime" &&
              !client.alertas.includes("Mudar Regime")
            )
              client.alertas.push("Mudar Regime");
          }
        } else {
          client.economiaPotencial = 0;
          client.score = 0;
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600">
              Gerencie seus clientes e planejamentos fiscais
            </p>
          </div>
                    ...
                    {clients.map((client) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 font-medium text-slate-700">
                          R${" "}
                          {Number(client.revenueLast12m || 0).toLocaleString(
                            "pt-BR",
                            { maximumFractionDigits: 0 },
                          )}
                        </td>
                        ...
                        <td className="px-4 py-4">
                          {client.economiaPotencial > 0 ? (
                            <span className="text-green-600 font-bold flex items-center">
                              ...
                              R${" "}
                              {client.economiaPotencial.toLocaleString(
                                "pt-BR",
                                { maximumFractionDigits: 0 },
                              )}
                              /ano
                          ...
                          <div className="flex flex-col gap-1">
                            {client.alertas?.map(
                              (alerta: string, idx: number) => (
                                <span
                                  key={idx}
                                  className="flex items-center text-[10px] font-bold text-red-700 bg-red-100 px-2 py-0.5 rounded w-max"
                                >
